...
def _build_instance_runtime_configs(instances, args, dtype_to_bits):
    ...
    for instance_id, instance in enumerate(instances):
        ...
        runtime_configs.append({
            ...
            "dtype": dtype,
            # dtype names the *weight* precision (it also selects the perf
            # variant folder). Activations — and the KV cache under
            # kv_cache_dtype=auto — stay in the compute precision, which is
            # bf16/fp16 even for fp8/int8 weight checkpoints (quantized GEMMs
            # dequantize per-tile; TP collectives and the latent/KV cache hold
            # 16-bit values). Keeping a single fp would halve every comm size
            # and double the apparent KV capacity for fp8 models, so split:
            #   weight_fp -> weight sizing, fp (>=16) -> activation/comm/KV.
            "weight_fp": dtype_to_bits[dtype],
            "fp": max(dtype_to_bits[dtype], 16),
