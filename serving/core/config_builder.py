...
def _resolve_parallelism(instance, model_config):
    ...
    # ``num_experts`` key, or the DeepSeek/GLM ``n_routed_experts`` key — HF
    # naming varies per model family and the profiler's configs track upstream.
    # (Must match trace_generator's gate detection and memory_model.is_moe,
    # which both include the n_routed_experts fallback.)
    is_moe = (
        'num_local_experts' in model_config
        or 'num_experts' in model_config
